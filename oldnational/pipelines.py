...
class OoldnationalPipeline:

    # Database setup
    conn = sqlite3.connect('oldnational.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `oldnational`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM oldnational WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO oldnational (date, title, link, content)"
